...
def register(cb):
    cb(CopyMod())
class CopyMod(loader.Module):
    """Полное копирование юзера(ава, имя|фамилия, био)"""
    strings = {'name': 'RakenCopyMod'}
    def __init__(self):
    ...
    async def cucmd(self, message):
        ...
        if not s:
            for i in range(0,11):
                await message.edit(f"Узнаём информацию об аккаунте... {i*10}%\n[{(i*'⬜️').ljust(10, '⬛️')}]")
                await sleep(0.3)
        if a:
        ...
        if not s: await message.edit("Скачиваем аватарку... 35%\n[⬜️⬜️⬜️⬛️⬛️⬛️⬛️⬛️⬛️⬛️]")
